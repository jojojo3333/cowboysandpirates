...
```
assets/ship/     ship plates and their derived normal maps, one set per ship
assets/crew/     rendered crew sprite sheets
assets/ui/       panels, borders, icons, HUD chrome
assets/audio/    sound and music (nothing here yet)
tools/crew_src/  3D render sources — never loaded by the game
```

Anything the game loads at runtime belongs under `assets/`. Render *sources* —
the `.glb` models — live under `tools/`, because they are build inputs rather
than game data and should not be shipped in the web export.
